...
N = 1024
generations = 1000  # Adjust for longer runs
cp_bias = 0.0245
...
# Gazebo Launch (via subprocess - Launch TurtleBot in Gazebo)
def launch_gazebo():
    subprocess.Popen(["roslaunch", "turtlebot3_gazebo", "turtlebot3_world.launch"])  # Replace with your Gazebo world
...
# Evolution Loop with ROS Control
fitness_history = []
...
for gen in range(1, generations + 1):
    ...
    # ROS Control: Map ALife to Robot (Average fitness to speed, decisions to turn)
    twist = Twist()
    twist.linear.x = fitness.mean().item()  # Speed based on fitness
    twist.angular.z = np.mean(decisions) * 0.5  # Turn based on collective decision
    pub.publish(twist)
    rate.sleep()
